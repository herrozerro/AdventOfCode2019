...
void Main()
{
	var bodies = GetData().Select(x => new body() { ID = x.Split(')')[1].Trim(), ParentID = x.Split(')')[0].Trim()}).ToList();

	for (int i = 0; i < bodies.Count(); i++)
	{
		var b = bodies[i];
		var children = bodies.Where(bo => bo.ParentID == b.ID).ToList();
		b.ChildrenBodies.AddRange(children);
		var parent = bodies.FirstOrDefault(x=>x.ID == b.ParentID);
		b.ParentBody = parent;
	}
	...
	{	
		var ancestors = b.Ancestors(bo => bo.ParentBody).Count() + 1;
	...
	var myAncestors = new List<string>();
...
public class body{
	...
	public List<body> ChildrenBodies {get;set;} = new List<body>();
}
